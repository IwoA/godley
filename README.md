...
sensivity <- function(model, x){
  ...
  create_sensitivity(model,
                        variable = x$name, lower = x$from, upper = x$to, step = x$step) |>
...
```         
plt <- purrr::map(result, \(x) plot_simulation_g(model = x, scenario = "sensitivity", take_all = TRUE, to = "2028-01-01", expressions = c("Y")))
plts <- patchwork::wrap_plots(plt, nrow = 2)
plts
```
